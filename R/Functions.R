...
{
  stop("This function does not currently work. Sorry!");
  prefix = as.character(prefix[1]);
  if (nchar(prefix)!=2)
    stop(paste("Module eigengene prefix is currently restricted to be two characters long.",
               "Recommended values are PC and ME"));
  .moduleColorOptions$MEprefix = prefix;
}
...
moduleColor.version = function()
{
  .moduleColorOptions$version;
}

moduleColor.revisionDate = function()
{
  .moduleColorOptions$revisionDate;
}
    ...
    if (class(pc)=='try-error')
    {
      ...
      if (subHubs)
      {
        ...
        pc = try( 
        {
          ...
          kIM = (apply(modAdj, 1, sum, na.rm = TRUE) - 1)^3;
          kIM[is.na(kIM)] = 0;
          hub = which.max(kIM)
                    ...
                    useSets = NULL, verbose = 0, indent = 0)
{
  ...
  for (set in useSets) 
  {
    ...
    names(orderedMEs[[set]]$data) = names(MEs[[set]]$data)[order];
...
.clustOrder = function(distM, greyLast = TRUE, 
                       greyName = paste(moduleColor.getMEprefix(), "grey", sep=""))
{
  greyInd = match(greyName, names(distM));
  if (greyLast && !is.na(greyInd)) 
  {
     clusterMEs = (greyName!=names(distM));
     if (sum(clusterMEs)>1)
     {
       h = hclust(as.dist(distM[clusterMEs, clusterMEs]), method = "average");
       order = h$order;
       if (sum(order>=greyInd)>0) order[order>=greyInd] = order[order>=greyInd]+1;
       order = c(order, greyInd);
     } else if (length(names(distM))>1) 
     {
       if (greyInd==1)
       {
         order = c(2, 1)
     ...
     if (length(distM)>1)
     {
       h = hclust(as.dist(distM), method = "average");
       order = h$order;
     } else order = 1;
  }
  order;
